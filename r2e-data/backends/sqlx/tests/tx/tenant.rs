...
impl ItemController {
    #[post("/")]
    async fn create(&self, #[managed] tx: &mut TenantTx<'_, Sqlite>) -> Result<String, HttpError> {
    ...
    #[get("/")]
    async fn list(&self, #[managed] tx: &mut TenantTx<'_, Sqlite>) -> Result<String, HttpError> {
...
impl MemoController {
    #[get("/")]
    async fn whoami(&self, #[managed] tx: &mut TenantTx<'_, Sqlite>) -> String {
...
impl GuardedController {
    #[get("/")]
    #[guard(Deny("nope"))]
    async fn blocked(&self, #[managed] tx: &mut TenantTx<'_, Sqlite>) -> String {
...
impl Directory {
    ...
    async fn provision() -> Self {
        ...
        for tenant in ["acme", "globex"] {
            let url = sqlite_file_url(tenant);
            ...
            pool.close().await;
            dsns.insert(tenant.to_string(), url);
        }
        Self {
            dsns,
    ...
    fn source(&self) -> PoolSource<Sqlite> {
        let dsns = self.dsns.clone();
        let lookups = Arc::clone(&self.lookups);
        PoolSource::<Sqlite>::sync(move |tenant: &TenantId| {
