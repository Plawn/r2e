// Canonical example-multi-tenant application source.
//
...
const DEMO_SECRET: &[u8] = b"multi-tenant-secret-change-in-production";
...
impl App for MultiTenantApp {
    ...
    async fn setup() -> AppEnv {
        ...
        println!(
            "Tenant acme (user):  {}",
            generate_token(DEMO_SECRET, "alice", "acme", &["user"])
        );
        println!(
            "Tenant acme (admin): {}",
            generate_token(DEMO_SECRET, "bob", "acme", &["user", "admin"])
        );
        println!(
            "Tenant globex (user): {}",
            generate_token(DEMO_SECRET, "charlie", "globex", &["user"])
        );
        println!(
            "Super-admin:          {}",
            generate_token(DEMO_SECRET, "root", "system", &["super-admin"])
        );
        println!();
