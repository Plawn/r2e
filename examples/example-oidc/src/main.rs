...
/// The canonical application blueprint. `r2e::launch` (production + dev
/// hot-reload) drives this single [`App`] impl: [`App::setup`] builds the
/// long-lived environment once, [`App::build`] assembles the app on a fresh
/// builder per patch.
pub struct OidcApp;
...
// ---------------------------------------------------------------------------
// Thin binary entry point — `launch` serves via serve_auto() (default
// 0.0.0.0:3000) and, under the `dev-reload` feature, hot-reloads the app.
// ---------------------------------------------------------------------------
