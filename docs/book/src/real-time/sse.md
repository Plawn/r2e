...
Build a single `SseBroadcaster` and provide it as a bean during app startup:
...
Any service or controller that injects `SseBroadcaster` (by type) shares the same channel.
