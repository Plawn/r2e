...
Under `r2e dev` the gRPC port survives hot-patch cycles like the HTTP one: the server is re-spawned on every cycle from the same bound socket (the dev listener store keys sockets by owner, so gRPC and HTTP on the same address string never share one), and the previous cycle's server stops accepting the moment the new one takes the socket, so no connection is answered with stale services. In-flight requests on the old server finish on the old routes.
