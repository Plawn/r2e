...
- `bind_tcp(addr)` — bind a listener for a transport the hook serves on its own port. A plain bind in production; inside the `r2e dev` hot-patch loop it goes through the same process-global listener store as the HTTP port, so the socket (and the port, even for `:0`) carries over between cycles.
...
`on_serve` hooks run **once per process**: an `r2e dev` hot patch drops the previous `run()` future (cancelling its shutdown token, and with it every task the hooks tracked) and the rebuilt cycle skips the startup lifecycle. A transport that owns a port would go silent after the first patch, so it registers with `on_serve_each_cycle` instead — same signature, but it re-runs on every cycle (still exactly once in production) and re-serves the port it gets from `bind_tcp`. The separate-port gRPC server is the reference user (task #997). Such a hook must be safe to run again.
