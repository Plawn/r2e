...
- `bind_tcp(owner, addr)` — async; binds a listener for a transport the hook serves on its own port (`owner` is a namespace such as `"grpc"`). Returns a `BoundListener { listener, handover }`. A plain bind in production; inside the `r2e dev` hot-patch loop it goes through the same process-global listener store as the HTTP port, keyed by `(owner, addr)`, so the socket (and the port, even for `:0`) carries over between cycles and two owners asking for the same address never share one. Taking the socket again cancels the previous cycle's `handover` token **and waits for that holder to release the socket** (bounded to 5 s; a stuck holder is logged and overridden): serve through `bound.into_incoming(shutdown)` — a `Stream<Item = io::Result<TcpStream>>` that checks shutdown **and** handover before every accept, ends when either fires, and releases the socket when it ends or is dropped. That is what makes the handover acknowledged: the old server has stopped accepting before the new cycle even holds the socket. `bound.stop_signal(shutdown)` is the same stop as a future (tonic's graceful-shutdown trigger). The `bind_tcp` future borrows nothing — move it into the task you `track`.
