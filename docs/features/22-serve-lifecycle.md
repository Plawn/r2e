...
- `bind_tcp(owner, addr)` — async; binds a listener for a transport the hook serves on its own port (`owner` is a namespace such as `"grpc"`). Returns a `BoundListener { listener, handover }`. A plain bind in production; inside the `r2e dev` hot-patch loop it goes through the same process-global listener store as the HTTP port, keyed by `(owner, addr)`, so the socket (and the port, even for `:0`) carries over between cycles and two owners asking for the same address never share one. Taking the socket again cancels the previous cycle's `handover` token **and waits for that holder to release the socket**: serve through `bound.into_incoming(shutdown)` — a `Stream<Item = io::Result<TcpStream>>` that checks shutdown **and** handover before every accept, ends when either fires, and releases the socket when it ends or is dropped. `bound.stop_signal(shutdown)` is the same stop as a future (tonic's graceful-shutdown trigger) and releases the socket the moment it resolves, so a server that breaks on the signal and then drains its connections does not delay the next cycle. That is what makes the handover acknowledged: the old server has stopped accepting before the new cycle even holds the socket. The wait is fail-open — bounded to 5 s, after which a holder that never acknowledged is logged and overridden (the only case where it could still take a queued connection). The `bind_tcp` future borrows nothing — move it into the task you `track`.
...
`on_serve` hooks run **once per hot-reload process**: an `r2e dev` hot patch drops the previous `run()` future (cancelling its shutdown token, which the tracked tasks observe and stop on — they are detached, not aborted) and the rebuilt cycle skips the startup lifecycle. (In production every `run()` is a fresh process, so the distinction does not arise.) A transport that owns a port would go silent after the first patch, so it registers with `on_serve_each_cycle` instead — same signature, but it re-runs on every cycle (still exactly once in production) and re-serves the port it gets from `bind_tcp`. The separate-port gRPC server is the reference user (task #997). Such a hook must be safe to run again.
...
- **Tests**: `TestApp::boot` runs the startup phase and keeps the app alive as a `RunningApp`, so `app.shutdown().await` executes this whole sequence under the app's own `drain_timeout` and `shutdown_grace_period`. A server from `app.serve()` runs on the tracked lane and drains with it — and drains under `drain_timeout` whichever trigger fired, so dropping the `TestServer` before the app is bounded too. What an in-process start does **not** do: (a) the plugin **serve hooks** — they bind ports (separate-port gRPC, MCP) and start the scheduler driver, so `ServeContext::track` work is the part of step 4 a test does not exercise; (b) SO_REUSEPORT sharded serving — no worker runtimes are spawned, which is why a registered `per_worker_service()` is a boot error there instead of a silently dead service (an invalid `server.workers` is a boot error on both paths); (c) QUIC/HTTP3 serving. And `shutdown()` mirrors the **signal** path: it leaves `StopHandle` untouched, exactly as SIGTERM does, so `is_stopped()` reads `false` inside `on_drain` unless the test fires `app.stop_handle().stop()` itself. See `docs/features/12-testing.md` and `r2e-test/tests/lifecycle.rs`.
