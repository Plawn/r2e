...
4. After the HTTP drain: tracked task handles (`spawn_service`, `ServeContext::track`) are awaited **concurrently**, each bounded on its own by `shutdown_grace_period` if set.
