...
- `FgaGuard` (r2e-openfga) — the spec is the `FgaCheck` builder value; the
  ...
  above. With a checked-in `.fga` model, start the builder with
  `FgaCheck::has(authz::document::viewer)` (typed, generated by
  `r2e_openfga::model!` — relation + object type compile-checked against the
  model); `FgaCheck::relation("viewer").on("document")` is the unchecked
  escape hatch for dynamic models.
