...
**Delivery semantics (distributed backends): at-least-once.** The broker copy is acked/committed only after all local handlers for the message resolve (`BackendState::dispatch_from_poller_tracked` → `DispatchCompletion::outcome()` → `DispatchOutcome::Ack`/`Nack`). Consequences: handlers MUST be idempotent (redelivery after a crash or a `Nack` is expected); a `Nack` whose payload was captured to a configured DLQ counts as processed (acked); messages that fail to deserialize (poison messages) are parked in the matching handlers' configured DLQs (when any) then acked, not redelivered; a panicking handler counts as `Nack`; `emit_and_wait` dispatches locally BEFORE publishing and records the outcome — a local nack makes the poller nack the broker copy for redelivery. Shared consume-loop machinery in `r2e_events::backend`: `WatermarkTracker` (per-partition commit watermark, nack-pinned, rebalance-aware via `remove_partition`) and `spawn_completion_forwarder` + `COMPLETION_CHANNEL_CAPACITY`/`COMPLETION_DRAIN_TIMEOUT` (pipelined ack decisions). `LocalEventBus` is in-process only — events don't survive a crash (no delivery guarantee across restarts).
