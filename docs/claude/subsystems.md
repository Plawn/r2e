...
`StatefulConstruct<S>` trait allows constructing a controller core from state alone (no HTTP context). Auto-generated by `#[controller]` for every controller core (always — the generated core holds only `#[inject]` app fields and `#[config]` fields; identity and request-scoped fields are stripped into the per-request façade). Required for:
- Consumer methods (`#[consumer]`) — event handlers that run outside HTTP requests
- Scheduled methods (`#[scheduled]`) — background tasks

Because the core never holds identity fields, `StatefulConstruct` is available even on controllers that use struct-level or param-level `#[inject(identity)]` — consumers and scheduled tasks operate on the core.
...
**Declarative consumers on controllers** via `#[consumer(bus = "field_name")]` in a `#[routes]` impl block. Consumers run on the controller core (which always implements `StatefulConstruct`), so they work regardless of any `#[inject(identity)]` fields. Consumers are registered automatically by `AppBuilder::register_controller`.
...
Scheduled tasks run on the controller core, which always implements `StatefulConstruct` (identity and request-scoped fields live only on the per-request façade). Controllers can therefore be used for scheduling regardless of any struct-level or param-level `#[inject(identity)]`.
