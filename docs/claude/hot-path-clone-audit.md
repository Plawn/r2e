...
| `r2e-observability` | was `src/middleware.rs:38,55,64` | `capture_headers: Vec<String>` deep-cloned in `Layer::layer` and in every `OtelTraceService::clone` | yes — same mechanism | **fixed** — `Arc<[HeaderName]>`, built once at boot. The layer itself moved to `r2e-core` (`HttpTraceLayer` + `HttpTraceSettings`); `r2e-observability` now only supplies the span shape (`src/span.rs`, `OtelRequestSpan`) |
...
| `r2e-core` | `src/runtime/layers.rs` `GraphKeepAlive`, `normalize_path_router`, `default_cors` | `Arc<BeanContext>` only | yes, but `Arc` | acceptable — deliberate, documented |
| `r2e-core` | `src/runtime/panic.rs` `CatchPanic::call` | `MatchedPath` + `Option<PanicHook>` per request | yes, both `Arc` bumps | acceptable — `MatchedPath` interns the route template as `Arc<str>`; the alternative (reading it only on the panic path) is impossible once the request has moved into the inner service |
