...
`resolvable` is generated by the derive alongside `from_config` and mirrors its
...
instead of re-implementing resolution. Manual `PropertyMeta` constructions can
use the baseline `PropertyMeta::in_config_map`.
