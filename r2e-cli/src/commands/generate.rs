...
pub fn grpc_service(name: &str, package: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Every service module reaches the generated proto code via
    // `super::proto` — make sure mod.rs declares it (same module the
    // `r2e add grpc` / `r2e new --grpc` scaffolds create).
    let mod_path = service_dir.join("mod.rs");
    if !mod_path.exists() {
        fs::write(
            &mod_path,
            "// Generated by build.rs (`r2e_grpc_build::compile()`) from proto/*.proto.\n\
    ...
    println!("  Next steps:");
    println!(
        "  1. Register in src/app.rs, inside App::build: {}",
        format!(".register_grpc_service::<{name}Service>()").cyan()
    );
    println!("  2. Run `cargo build` — build.rs picks up the new proto automatically");
    println!(
        "     (no build.rs yet? run {} first)",
        "r2e add grpc".cyan()
    );
...
fn grpc_service_rs(name: &str, package: &str, snake: &str) -> String {
    // A dotted proto package (`myapp.v1`) becomes nested modules (`myapp::v1`).
    let package = package.replace('.', "::");
    format!(
        r#"use r2e::prelude::*;

// The shared proto module lives in mod.rs (generated by build.rs from
// proto/*.proto — one Rust module per proto package).
use super::proto;
use proto::{package}::*;
...
#[grpc_routes(proto::{package}::{snake}_server::{name}, descriptor = proto::FILE_DESCRIPTOR_SET)]
impl {name}Service {{
    async fn get_{snake}(
        &self,
