...
pub fn cargo_toml(opts: &ProjectOptions) -> String {
    ...
    if opts.grpc {
        ...
        // Reflection is on by default: the descriptor set is wired
        // automatically by r2e-grpc-build, so `grpcurl list` just works.
        r2e_features.push("grpc-reflection");
    }
...
pub fn app_rs(opts: &ProjectOptions) -> String {
    ...
    if opts.grpc {
        imports.push_str("use r2e::r2e_grpc::{AppBuilderGrpcExt, GrpcServer};\n");
    }
    if opts.auth {
...
pub fn greeter_proto(project_name: &str) -> String {
    // Proto packages allow neither `-` nor other non-identifier characters.
    let package = crate_ident(project_name);
    format!(
        r#"syntax = "proto3";
...
pub fn build_rs() -> &'static str {
    ...
    // Compiles every .proto under proto/ and generates the aggregated module
    // consumed via `r2e::r2e_grpc::include_protos!()`. Dropping a new .proto
    // in proto/ is picked up automatically — no changes needed here.
    r2e_grpc_build::compile()
}
"#
}

/// gRPC module root (`src/grpc/mod.rs`): the single shared `proto` module
/// (every service file in the directory reaches it via `super::proto`) plus
/// the scaffolded service module. `r2e generate grpc-service` appends
/// further `pub mod <name>;` lines to this file.
pub fn grpc_mod_rs() -> &'static str {
    r#"// One Rust module per proto package, plus `FILE_DESCRIPTOR_SET` (used by
// gRPC server reflection) — all generated by build.rs from proto/*.proto.
// Shared by every service module in this directory (via `super::proto`).
pub mod proto {
...
/// Scaffolded gRPC service (`src/grpc/greeter.rs`), implementing the
/// `Greeter` service from `proto/greeter.proto`.
pub fn grpc_greeter_rs(project_name: &str) -> String {
    let package = crate_ident(project_name);
    format!(
        r#"use r2e::prelude::*;

use super::proto;
use proto::{package}::{{HelloReply, HelloRequest}};

#[controller]
pub struct GreeterService;

#[grpc_routes(proto::{package}::greeter_server::Greeter, descriptor = proto::FILE_DESCRIPTOR_SET)]
impl GreeterService {{
    async fn say_hello(
        &self,
