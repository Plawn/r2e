...
r2e-http        → HTTP abstraction layer. Sole owner of the axum dependency; re-exports Router, extractors, responses, middleware, routing, WebSocket, multipart, and QUIC/HTTP3 types. Owns `IntoHttpResponse` (R2E's response contract) + the `impl_into_response!` bridge macro, and `axum_compat` (the explicit raw-axum escape hatch). QUIC support (feature `quic`) provides HTTP/3 via h3+h3-quinn (bridged to axum Router) and raw QUIC streams via quinn.
