...
r2e-core        → Also owns Page/Pageable and the cancellation-safe ManagedResource lifecycle.
  r2e-data/backends/sqlx   → Managed SQLx transactions (SQLite/Postgres/MySQL). `SqlxDataSource<DB, Tag>` pre-state plugin: `datasource.*` config → connected `DbPool<DB, Tag>` bean, `migrate-at-start` runs the attached `sqlx::migrate!` inside `build_state()`, pool closed at shutdown; named datasources via `datasource_tag!`. Feature `tenant`: per-tenant pools (`TenantPools<DB>`, `PoolSource<DB>`) + `TenantTx<'_, DB>`.
  r2e-data/backends/diesel → Managed Diesel transactions (SQLite/Postgres/MySQL). `DieselDataSource<Conn, Tag>` mirrors `SqlxDataSource` (same `datasource.*` section, `embed_migrations!`). Feature `tenant`: per-tenant r2d2 pools (`TenantPools<Conn>`, `PoolSource<Conn>`) + `TenantTx<Conn>`.
