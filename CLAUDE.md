...
r2e-core        → Also owns Page/Pageable and the cancellation-safe ManagedResource lifecycle.
  r2e-data/backends/sqlx   → Managed SQLx transactions (SQLite/Postgres/MySQL). Feature `tenant`: per-tenant pools (`TenantPools<DB>`, `PoolSource<DB>`) + `TenantTx<'_, DB>`.
  r2e-data/backends/diesel → Managed Diesel transactions (SQLite/Postgres/MySQL). Feature `tenant`: per-tenant r2d2 pools (`TenantPools<Conn>`, `PoolSource<Conn>`) + `TenantTx<Conn>`.
...
Dependency flow: `r2e-http` ← `r2e-macros` ← `r2e-core` ← `r2e-security` / `r2e-events` / `r2e-scheduler` / `r2e-devtools` / `r2e-static` / `r2e-tenant` / `r2e-data-sqlx` / `r2e-data-diesel` / other integrations ← `r2e` ← applications. The data backends' `tenant` feature adds `r2e-tenant`, so `r2e-tenant` precedes them.
...
| `Plugin`, `PreStatePlugin`, `.plugin()`, `.with()`, `Provided`/`PluginProvisions`, `Deps`, `configure`, plugin `Config`/`CONFIG_PREFIX`/`PluginConfig`, `PluginInstallContext`, `DeferredAction`/`DeferredContext`, `store_data`/plugin data, `should_be_last`, writing a new plugin | `docs/claude/plugins.md` |
