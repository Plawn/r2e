...
pub struct PropertyMeta {
    ...
    pub is_section: bool,
    /// Presence oracle for this property, chosen by
    /// `#[derive(ConfigProperties)]` alongside `from_config`: `true` when
    /// `from_config` would find an explicit value (same sources, same order —
    /// config map, then a custom `#[config(env = "...")]` var). Defaults are
...
impl PropertyMeta {
    /// Whether `from_config` would find an explicit value for this property.
    pub fn is_resolvable(&self, config: &R2eConfig) -> bool {
        (self.resolvable)(config, self)
    }

    /// Standard-sources probe: the property resolves iff its `full_key` is in
    /// the config map, or the custom env var carried in `env_var` is set.
    /// Reading `env_var` as data (rather than baking the var name into a
    /// generated closure) keeps the probe and the `env_var` field impossible
    /// to desync. The derive uses this for every non-section property; manual
    /// [`ConfigProperties`] impls that build `PropertyMeta` by hand should
    /// use it too unless the property has an exotic resolution source.
    pub fn standard_sources(config: &R2eConfig, meta: &PropertyMeta) -> bool {
        config.contains_key(&meta.full_key)
            || meta
                .env_var
                .as_deref()
