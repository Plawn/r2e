...
/// An on-start hook source: reads its target bean by type from the resolved
/// graph (override-aware, like post-construct hooks) and returns that bean's
/// `#[on_start]` hooks as `(order, hook)` pairs. Drained by `build_state()`
/// into the builder, which merges every bean's and controller's hooks into a
/// single order-sorted list run at server startup.
pub(crate) type OnStartSourceHook =
    Box<dyn FnOnce(&BeanContext) -> Vec<(i32, crate::beans::OnStartHook)> + Send>;
...
pub struct BeanRegistry {
    ...
    /// On-start hook sources queued by `after_register` (generated by
    /// `#[bean]` when `#[on_start]` methods are present). Taken by the builder
    /// before `resolve` and run against the resolved graph — see
    /// [`register_on_start`](Self::register_on_start). Keyed by the bean
    /// `TypeId` (one entry per type — the default/override pattern registers
    /// twice but must run its startup hooks once).
    pub(super) on_start_hooks: Vec<(TypeId, &'static str, OnStartSourceHook)>,
