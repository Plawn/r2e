...
pub trait Controller<T: Clone + Send + Sync + 'static> {
    ...
    /// Return a CONNECT tunnel handler, if this controller handles CONNECT.
    ///
    /// Generated by the `#[routes]` macro when a `#[connect]` method is present.
    /// The builder collects these and installs a Tower layer that intercepts
    /// `Method::CONNECT` before the Axum router.
    #[cfg(feature = "proxy")]
    fn connect_handler() -> Option<crate::proxy::ConnectHandlerFn<T>> {
        None
    }
}
