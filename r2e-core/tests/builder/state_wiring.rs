//! `AppBuilder` end-to-end wiring: materializing the bean graph into the HList
//! state (`provide` / `bean` / `async bean` / `producer` / config injection),
//! conditional availability, profile gating, layer + hook registration, and
//! raw route registration.
...
use crate::support::{raw_get_with, send_get};
