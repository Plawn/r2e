...
use r2e_openfga::{FgaClient, GrpcBackend, OpenFgaConfig, OpenFgaRegistry};
...
r2e_openfga::model!(pub mod tiny_authz = inline r#"model
  schema 1.1
type user"#);
```
...
r2e_openfga::model!(pub mod authz = "fga/model.fga");   // or `use crate::authz;`
...
impl DocumentController {
    ...
    async fn share(&self, Path((doc_id, user_id)): Path<(String, String)>)
        ...
        let bad = |e: r2e_openfga::InvalidObjectId| HttpError::bad_request(e.to_string());
        let fail = |e: r2e_openfga::OpenFgaError| HttpError::internal(e.to_string());
