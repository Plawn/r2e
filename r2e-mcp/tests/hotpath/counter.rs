...
//!
//! A copy of `examples/example-app/tests/hotpath/counter.rs` — a global
//! allocator can only be installed by the binary that measures with it, so
//! each hot-path guard target owns one. Keep the two in sync.
//!
...
pub fn runtime() -> r2e_core::rt::Runtime {
...
/// Assert that a per-request cost did not grow with the size of some
/// app-lifetime or per-caller data that must be shared, not copied
/// (an immutable config, a validated claims tree, …).
///
