...
- Satisfies C1 textually (one source), but two compilations of the same code
  (symbol duplication, `#[module]`/macro side effects — bin and lib are separate
  artifacts, so likely fine).
- Risks: path-attribute inclusion is brittle with `mod` trees; proc-macros that
  assume crate-root paths keep working (same source, different crate for
  `crate::` — should be fine); rust-analyzer sees the code twice; dx must watch
  the right files (it watches the package dir — OK).
